...
def thuc_hien_cong_viec():
	...
	try:
		...
		list_layout_excel_old = sht.range("B"+str(first_row)+":B"+str(last_row)).value
		list_layout_excel = [x for x in list_layout_excel_old if x is not None]
		list_TENBV1_excel_old = sht.range("C"+str(first_row)+":C"+str(last_row)).value
		list_TENBV1_excel = [x for x in list_TENBV1_excel_old if x is not None]
		list_TENBV2_excel_old = sht.range("D"+str(first_row)+":D"+str(last_row)).value
		...
		list_LCS_excel_old = sht.range("G"+str(first_row)+":G"+str(last_row)).value
		list_LCS_excel = [x for x in list_LCS_excel_old if x is not None]
		list_BVS_excel_old = sht.range("H"+str(first_row)+":H"+str(last_row)).value
						...
						for element in list_layouts[i].Block:
							if element.EntityName == "AcDbBlockReference" and element.HasAttributes and element.name == "DP_NDBV":
								...
								for att in list_att:
									...
									if att.TagString == "TENBV1":
										att.TextString = list_TENBV1_excel[j]
									elif att.TagString == "TENBV2":
								...
								thong_bao = "-Xong khung tên của layout:" + str(list_BVS_excel[j])+"\n"
								dem = dem + 1
								result_text.insert(tk.END,thong_bao)
								print(thong_bao)
				# print("----------------")
			# thong_bao1 = "-Xong khung tên toàn bộ file\n"
			thong_bao1 = "-Xong khung tên của "+str(dem)+"/"+str(len(list_layouts))+" layouts"
...
button_chon_tep = tk.Button(root, text="Chọn ACad", command=chon_tep_cad, bg="#4caf50", fg="white", font=("Arial", 10, "bold"))  # Thiết lập màu cho nút "Chọn"
button_chon_tep.grid(row=0, column=2, pady=0, padx=0)
# Tạo nút "Chọn" cho ô nhập liệu 2
button_chon_tep = tk.Button(root, text="Chọn Excel", command=chon_tep_excel, bg="#4caf50", fg="white", font=("Arial", 10, "bold"))  # Thiết lập màu cho nút "Chọn"
button_chon_tep.grid(row=1, column=2, pady=0, padx=0)

# Tạo nút "Thực hiện công việc"
button_thuc_hien = tk.Button(root, text="Update Khung", command=thuc_hien_cong_viec, bg="#2196f3", fg="white", font=("Arial", 10, "bold"))  # Thiết lập màu cho nút "Thực hiện công việc"
